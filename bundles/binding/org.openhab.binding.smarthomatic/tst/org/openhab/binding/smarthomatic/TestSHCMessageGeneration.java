...
public class TestSHCMessageGeneration {
	...
	public void testAllDeviceIds() {
		for (int deviceid = 0; deviceid < 4096; deviceid++) {
			...
			Assert.assertEquals(deviceid,
					Integer.parseInt(cmd.substring(5, 9), 16));
		}
	}
	...
	public void testAllMessageGroupIds() {
		...
		for (int msggroupid = 0; msggroupid < 256; msggroupid++) {
			...
			cmd = base.prepareCommand(0, msggroupid, msgid, 0, new DecimalType(
					0));
			if (is_supported) {
				int strlen = cmd.length();
				Assert.assertEquals("Length = " + strlen, true, strlen > 0);
				Assert.assertEquals(msggroupid,
						Integer.parseInt(cmd.substring(9, 11), 16));
			} else {
		...
		for (int msgid = 0; msgid < 256; msgid++) {
			...
			if (is_supported) {
				...
				Assert.assertEquals(msgid,
						Integer.parseInt(cmd.substring(11, 13), 16));
			} else {
				Assert.assertEquals("", cmd);
			}
		}
	...
	public void testGPIODigitalPortAllOff() {
		...
		for (int key: test_msg.keySet()) {
			...
			Assert.assertEquals(test_msg.get(key), cmd);
		}
	}
	...
	public void testGPIODigitalPin() {
		for (int pin = 0; pin < 8; pin++) {
			...
			Assert.assertEquals(pin * 2, Integer.parseInt(cmd.substring(13, 14), 16));
			Assert.assertEquals(14, cmd.length());
			// Pin i = On
			cmd = base.prepareCommand(14, 1, 5, 0, new DecimalType(pin * 2 + 1));
			Assert.assertEquals(pin * 2 + 1, Integer.parseInt(cmd.substring(13, 14), 16));
			Assert.assertEquals(14, cmd.length());
		}
	}
